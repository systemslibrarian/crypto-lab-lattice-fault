 ...
 *    files the node under `incomplete`. This page is full of them: `<body>`
 *    paints a `radial-gradient` glow over `--bg`, every `button` is a
 *    `linear-gradient`, and almost every surface tint (`.primer`,
 *    `.attacker-story`, `.lp-take`, `.algebra-walk`, the inline `code` chips)
 *    is a `color-mix(in srgb, ...)` wash that composites onto whatever panel is
 *    behind it rather than onto a flat colour axe can read.
 ...
 *    `button:disabled { opacity: .55 }`, which the busy-state buttons take
 *    while CPA / recovery / lattice reduction runs. A disabled control is
 *    exempt from 1.4.3 (see `inactive` below), but the modelling is kept
 *    because it is what makes the walk correct rather than accidentally right.
 *
 ...
 * not behind it — this page puts `#theme-toggle` at `position: absolute` inside
 * `.cl-hero` for exactly that reason. So an ancestor's own paint is applied
 * only when its border box actually intersects the text's box; a partial
 * intersection still counts, so the judgement stays worst-case. Opacity is
 * unconditional either way — an opacity group fades its whole subtree wherever
 * that subtree happens to paint.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The Attack-5
 *    countermeasure table sits in `.comparison-table-wrap { overflow-x: auto }`,
 *    the `c⁻¹z` relation in `.formula-line { overflow-x: auto }`, and the
 *    warm-up PIN pseudocode in `.warmup-code { overflow-x: auto }` — all three
 *    clip at 380px. Content scrolled past the client box is not dimmed or
 *    partly drawn — it is absent from the frame, and asking what colour it sits
 *    on has no answer. Its rect is still to the right of every ancestor's box,
 *    so the ancestor walk would find nothing behind it and fall through to
 ...
 *    on-ramp primer's `.lp-figure` diagram does exactly this: it draws
 *    `<rect class="lp-svg-box">` and then the two product labels
 *    (`.lp-svg-p`, `.lp-svg-hot`) as FOLLOWING siblings inside it, so their
 *    backdrop is a preceding sibling shape, not an ancestor. An ancestor-only
 *    walk would composite those labels onto the svg's own transparent
 *    background and report a ratio nothing on screen has. So for SVG text, any
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * into surfaces with `color-mix(in srgb, ...)` everywhere — the hero's
     * "why it matters" box, `.primer`, `.attacker-story`, `.algebra-walk`, the
     * inline `code` chips — and Chromium reports those
     ...
     * that spans its element, but `<body>` here paints
     * `radial-gradient(circle at top, rgba(0,255,102,.12), transparent 30%)` —
     * a decorative glow that has faded to nothing a third of the way down a page
     * many viewports tall. Judging the footer by the top-of-page stop invents a
     * backdrop the footer text never sits on. So each gradient is *sampled* at
     ...
     *
     * This page renders four `<p>` rows per butterfly card across the whole
     * NTT-stage grid, plus the countermeasure table and four long reference
     * lists, and every one of them walks the same handful of ancestors. Without
     ...
     * surface it happens to sit on and INVENTS failures — measured elsewhere in
     * this fleet at 1.15:1 for a card's screen-reader-only label. This lab has
     * no `.sr-only` recipe today; the guard is kept because it is cheap, silent
     * when unused, and the idiom is one copy-paste away. Deliberately narrow —
     * only a ZERO-area clip qualifies, so a real partial clip is still measured.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has four disclosures: the `#lattice-primer`
      // on-ramp (open by default) and the three `.attack-reveal` wrappers
      // holding Attacks 2, 3 and 4 (closed by default). Every one of those open
      // and closed states is a real state the gate drives and scans explicitly.
      ...
      // the WCAG-sanctioned "visually hidden until focused" idiom, used by the
      // shared header's `.cl-skip-link`, which parks at `top: -3rem` and slides
     ...
     * stricter than axe on decorative content. Honour the same boundary — the
     * shared header marks its hamburger and GitHub glyphs `aria-hidden`, and
     * their `currentColor` strokes are a 1.4.11 question, not a 1.4.3 one.
     */
     ...
     * against the panel — a ratio describing ink that was never laid down. The
     * `.lp-figure` diagram here is static markup and trips none of it; the
     * guard is kept because it is cheap and silent when unused.
     */
     ...
     *
     * Here `:root` paints no background at all, so the canvas colour comes from
     * `<body>` — its `radial-gradient` glow composited over `var(--bg)`. Every
     * `--panel` / `--box` surface on this page is translucent
     * (`rgba(9,25,14,.92)`, `rgba(4,19,10,.72)`), so essentially every text
     * measurement on the page reaches this fallback rather than stopping at an
     * opaque ancestor.
     */
